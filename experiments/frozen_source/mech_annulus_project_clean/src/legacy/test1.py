...
def build_hash_tables(points, filtered_candidates, num_tables, num_planes, dimension):
    ...
    for index in range(num_tables):
        ...
        for point_id in filtered_candidates:
            ...
            hash_table[index][hash_value].append(point_id)  # 点的索引存入对应的哈希桶
...
def get_true_neighbors(query_point, points, radius):
    ...
    true_neighbors = np.where(distances <= radius)
...
def compute_recall(true_neighbors, retrieved_neighbors):
    ...
    计算召回率
    """
    print(len(set(true_neighbors[0]) & set(retrieved_neighbors)),  len(true_neighbors[0]))
    return len(set(true_neighbors[0]) & set(retrieved_neighbors))  / len(true_neighbors[0]) if len(true_neighbors[0]) > 0 else 0
