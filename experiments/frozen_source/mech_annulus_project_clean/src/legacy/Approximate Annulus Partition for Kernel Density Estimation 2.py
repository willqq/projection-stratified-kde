...
class CrossPolytopeLSH:
    def __init__(self, d, k,n):
        ...
        self.k = k
        self.n = n
        self.random_rotations=self._generate_random_rotations()

    def _generate_random_rotations(self):
        """
        生成 k 个随机旋转矩阵。
        """
        random_rotations = []
        for _ in range(self.n):
            ...
            # 按行归一化
            Q = A / row_norms
            # Q, _ = np.linalg.qr(A)  # QR 分解得到正交矩阵
            random_rotations.append(Q)
        return random_rotations

    def hash(self, v):
        """
        计算数据点 v 的哈希值。
        :param v: 数据点（numpy 数组）
        :return: 哈希值（整数列表）
        """
        hash_values = []
        for index,R in enumerate(self.random_rotations):

            v_rotated = np.dot(R, v)  # 旋转数据点
            max_index = np.argmax(np.abs(v_rotated))  # 找到绝对值最大的维度
            sign = 1 if v_rotated[max_index] >= 0 else -1  # 确定符号
            hash_value = str(max_index * sign)+"_"+str(index)  # 哈希值
            hash_values.append(hash_value)
        return tuple(hash_values)

    def build_hash_table(self, data,filtered_candidates):
        """
        构建哈希表。
        ...
        """
        hash_table = {}
        for idx, v in enumerate(filtered_candidates):
            hash_values = self.hash(data[v])
            for hash_value1 in hash_values:
                if hash_value1 not in hash_table:
                    hash_table[hash_value1] = []
                    print(hash_value1)
                hash_table[hash_value1].append(v)
        return hash_table

    def query(self, q, hash_table,points):
        """
        查询最近邻。
        :param q: 查询点（numpy 数组）
        ...
        for hash_value in hash_values:
            dist = []
            print("hash_table.get(hash_value, [])",len(hash_table),len(hash_table.get(hash_value, [])))
            hash.extend(hash_table.get(hash_value, []))
            # for hash_point in  hash_table.get(hash_value, []):
            #     candidate_point=points[hash_point]
            #     distance = -np.linalg.norm(candidate_point - q)  # 计算欧式距离
            #     # print(distance)
            #     dist.append(distance)
            # res=topk_indices_numpy(dist, 50)
            # # print("res",res,len(hash_table.get(hash_value, [])))
            # for i in res:
            #     hash.append(hash_table.get(hash_value, [])[i])
        return hash
...
#     # 生成随机数据
#     np.random.seed(42)
#     data = np.random.randn(100, 10)  # 100 个 10 维数据点
#     query_point = np.random.randn(10)  # 查询点
#
#     # 初始化 Cross-Polytope LSH
#
#     # 构建哈希表
#     hash_table = lsh.build_hash_table(data)
#
#     # 查询最近邻
#     candidates = lsh.query(query_point, hash_table)
#     print("候选最近邻的索引:", candidates)
...
def generate_random_hyperplanes(dimension, num_planes):
    a=np.random.randn(num_planes, dimension)
    # print("a",a)
    return a
...
def topk_indices_numpy(lst, k):
    import heapq
    a=heapq.nlargest(k, range(len(lst)), lst.__getitem__)
    # topk_indices = topk_indices[np.argsort(-arr[topk_indices])]
    return a

def query_candidate_points(hash_candidate,query_hash_value, hash_tables, distance_tables, max_hamming_distance=1):
    """根据哈希值查询候选点"""
    candidate_points1 = set()
    candidate_points2= set()

    # 从每个哈希表中查询哈希值相同或汉明距离在阈值以内的点
    for table, distance_table in zip(hash_tables, distance_tables):
        if query_hash_value in table:
            # dist=[]
            # for hash_point in  table[query_hash_value]:
            #     candidate_point=points[hash_point]
            #     distance = -np.linalg.norm(candidate_point - query_point)  # 计算欧式距离
            #     # print(distance)
            #     dist.append(distance)
            # res=topk_indices_numpy(dist, 100)
            # for i in res:
            candidate_points1.update(hash_candidate)
            # candidate_points1.update(res)  # 查询哈希值相同的点
            print(len(table[query_hash_value]))
        ...
        #             candidate_points2.update(table[candidate_hash])  # 查询与给定哈希值汉明距离为 i 的点
    #两个集合求交集
    candidate_points= candidate_points1.union(candidate_points2)
...
def max_angle(query_point, radius):
    ...
    theta_max = np.arcsin(0.9*radius / norm_q)
    return theta_max
...
def query_with_max_angle(points, query_point, radius, sphere_table, k, num_planes, dim, max_hamming_distance=1):
    ...
    # print("len(query_point),len(filtered_candidates)",len(points),len(filtered_candidates))
    filtered_candidates = filter_candidates_by_distance(filtered_candidates, points, query_point, radius)
    # print("11len(query_point),len(filtered_candidates)",len(points),len(filtered_candidates))
    print(len(filtered_candidates),"len(filtered_candidates)")
    lsh = CrossPolytopeLSH(d=dim, k=num_planes,n=k)
    hash_tables = lsh.build_hash_table(points,filtered_candidates)
    # print("len(hash_tables)",len(hash_tables),hash_tables)
    # hash_candidate= lsh.query(query_point,hash_tables,points)
    ...
    for i, hyperplane_set in enumerate(hyperplane_sets):
        ...
        candidate_points.update(
            query_candidate_points(hash_candidate,query_hash_value, hash_tables, distance_tables, max_hamming_distance))
...
def annulus_monte_carlo_sample(query_point, candidates, points, mbegin=100, epsilon=0.5, alpha=0.5, beta=2.0, bandwidth=1.0):
    ...
    while True:
        ...
        sampled_points = np.random.choice(list(candidates), m_rest, replace=False)
...
def get_true_neighbors(query_point, points, radius):
    ...
    index = set(np.where(distances <= radius)[0])
    index = list(index)
    index.sort()
    return true_neighbors,index

def compute_recall(true_neighbors, retrieved_neighbors):
    """
    计算召回率
    """
    true_neighbors = set(true_neighbors)
    retrieved_neighbors= set(retrieved_neighbors)
    # print(true_neighbors & retrieved_neighbors)
    rec=len(true_neighbors & retrieved_neighbors)/len(true_neighbors) #if len(true_neighbors) > 0 else 0
    print("true_neighbors:",len(true_neighbors),len(retrieved_neighbors),rec)
...
def compute_precision(true_neighbors, retrieved_neighbors):
    ...
    true_neighbors = set(true_neighbors)
    retrieved_neighbors = set(retrieved_neighbors)
    prec=len(true_neighbors & retrieved_neighbors)/len(retrieved_neighbors) #if len(true_neighbors) > 0 else 0
    print("prec:", len(true_neighbors), len(retrieved_neighbors), prec)
    return prec
...
def plot_results(x, recall, precision):
...
# points = get_Amazon_data() #num_planes = 3
...
r1, r2, r3 = calculate_ring_parameters(query_point, points, 10, 20, 50)
radii = [r1, r2]
radius = r2
num_planes = 15
...
print("Traditional density's time:", (endtime - starttime))
print("---------------")
...
for k in k_values:
    ...
    true_neighbors,true_set = get_true_neighbors(query_point, points, radius)
    # print("True neighbors:", true_neighbors,radius)
    retrieved_neighbors = len(candidate_set)
    sampled_points_number = len(sampled_points)
    total_retrieved_neighbors = len(points)

    recall = compute_recall(true_set, candidate_set)
    precision = compute_precision(true_set, candidate_set)
...
# 绘制
plot_results(k_values, recall_values, precision_values)
