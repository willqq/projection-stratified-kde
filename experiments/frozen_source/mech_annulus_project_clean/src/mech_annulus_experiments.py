...
def max_angle_from_radius(q_norm: float, radius: float) -> float:
    if radius < 0 or q_norm < 0:
        raise ValueError("Norm and radius must be nonnegative")
    if q_norm == 0 or radius > q_norm:
        return math.pi
    return math.asin(min(1.0, radius / q_norm))
...
def distance_table_neighbor_codes(
    ...
    if distance_tables is not None and q_code in distance_tables[table_id]:
        ...
        return codes
    if hasattr(index, "query_stats"):
        index.query_stats["code_distance_checks"] += len(index.tables[table_id])
    return [
        int(code)
...
def hash_filter_candidates(
    ...
    if hasattr(index, "query_stats"):
        index.query_stats["hash_id_checks"] += len(ids) * len(q_codes)
...
def approx_fixed_radius_partition_radius_first(
    ...
    if variant in {"full", "no_distance"}:
        ...
        for outer_radius in bounds[1:]:
            ...
            # Zero reference vectors have no angle; use their radial distance.
            zero_ids = getattr(index, "zero_norm_ids", None)
            if zero_ids is None:
                zero_ids = set(np.flatnonzero(norms == 0).tolist())
            current_hash.difference_update(zero_ids)
            if q_norm <= outer_radius:
                current_hash.update(zero_ids)
