...
def formal(e,queries,rawids,variants,out,budgets,old_replay=None):
    out.mkdir(parents=True,exist_ok=False)
    save_json(out/'START.json',dict(created_unix=time.time(),source_commit=r.git_head(),script_sha256=r.sha(Path(__file__))))
    cc=control(e);rows=[];times=[];maxdiff=0.
...
def variants(freeze,oldcfg):
    # Keep only query parameters: the construction rule supplies the actual
    # nlist for each n (25 at n=1000), recorded in offline.json.
    result={'deann_original':{k:oldcfg[k] for k in ['nprobe','near_fraction']}};seen={(oldcfg['nprobe'],oldcfg['near_fraction'])}
